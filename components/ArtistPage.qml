...
Item {
    ...
    function artist_albums(artistId) {
        Scripts.artist_albums(artistId);
        Scripts.artist_songs(artistId);
        get_fav();
        get_lib();
    }

    function get_fav() {
        Scripts.get_fav_songs(uid, 'artist');
    }
    function get_lib() {
        Scripts.get_lib_songs(uid, 'artist');
    }

    function secCh(index) {
        if (index == '0') {
            artistsongs.visible = false;
            artistalbums.visible = true;
            indicatsec.visible = false;
            indicat.visible = true;
        } else if (index == '1') {
            artistsongs.visible = true;
            artistalbums.visible = false;
            indicatsec.visible = true;
    ...
    GridView {
        ...
        delegate: ListItem.Empty {
            width: artistalbums.cellWidth
            height: artistalbums.cellHeight
            MouseArea {
                width: parent.width - units.gu(2)
                height: albumcover.height + units.gu(2)
                onClicked: {
                    albumPage.title = title;
            ...
            Item {
                ...
                Text {
                    ...
                    width: parent.width - albumcover.width - units.gu(5)
                    id: albumtitle
                    text: title
    ...
    Scrollbar {
        flickableItem: artistalbums
    }

    Item {
        id: indicat
        anchors.centerIn: parent
        opacity: artistalbumsModel.count != 0 ? 0 : 1
    ...
    ListView {
        ...
        delegate: ListItem.Empty {
            ...
            MouseArea {
                ...
                onClicked: {
                    queueModel.clear();
                    for (var i = 0; i < artistsongsModel.count; i++) {
                        queueModel.append(artistsongsModel.get(i));
                    }
            ...
            Item {
                ...
                Rectangle {
                    ...
                    Flickable {
                        ...
                        Item {
                            id: buttons
                            width: active_download == 0 ? downloadButton.width + externalButton.width + lyricsButton.width + albumButton.width + favButton.width + collectionButton.width + pylButton.width + shareButton.width + buyButton.width + units.gu(32) + units.gu(6) : parent.width
                            height: parent.height
                            anchors.left: parent.left
                            anchors.leftMargin: units.gu(2)
                            Rectangle {
                            ...
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_fav == 1) {
                                            Scripts.remove_from_fav(id, 'artist');
                                        } else {
                                            Scripts.add_to_fav(id, 'artist');
                                        }
                                    }
                                }
                            }
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_lib == 1) {
                                            Scripts.remove_from_lib(id, albumId, artistId, uid, 'artist');
                                        } else {
                                            var song_data = [{
                                                'songID' : id,
                                                'songName' : title,
                                            ...
                                            Scripts.add_to_lib(song_data, 'artist');
                                        }
                                    }
                                }
                            }
    ...
    Scrollbar {
        flickableItem: artistsongs
    }

    Item {
        id: indicatsec
        anchors.centerIn: parent
        opacity: artistsongsModel.count != 0 ? 0 : 1
