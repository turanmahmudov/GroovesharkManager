...
Item {
    ...
    function popular() {
        Scripts.popular_songs();
        get_fav();
        get_lib();
    }

    function get_fav() {
        Scripts.get_fav_songs(uid, 'popular');
    }
    function get_lib() {
        Scripts.get_lib_songs(uid, 'popular');
    }
    ...
    Connections {
        ...
        onFinished: {
            ...
            var i;
            for (i = 0; i < popularsongsModel.count; i++) {
                if (popularsongsModel.get(i).id == active_download) {
                        ...
                        Item {
                            ...
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_fav == 1) {
                                            Scripts.remove_from_fav(id, 'popular');
                                        } else {
                                            Scripts.add_to_fav(id, 'popular');
                                        }
                                    }
                                }
                            }
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_lib == 1) {
                                            Scripts.remove_from_lib(id, albumId, artistId, uid, 'popular');
                                        } else {
                                            var song_data = [{
                                                'songID' : id,
                                                'songName' : title,
                                            ...
                                            Scripts.add_to_lib(song_data, 'popular');
                                        }
                                    }
                                }
                            }
    ...
    Scrollbar {
        flickableItem: popularsongs
    }

    Item {
        id: indicat
        anchors.centerIn: parent
        opacity: popularsongsModel.count != 0 ? 0 : 1
