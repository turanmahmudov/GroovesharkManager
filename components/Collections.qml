...
Item {
    ...
    function get_fav() {
        Scripts.get_fav_songs(uid, 'collection');
    }
    function get_lib() {
        Scripts.get_lib_songs(uid, 'collection');
        get_fav();
    }
    ...
    ListView {
            ...
            MouseArea {
                ...
                onClicked: {
                    ...
                    for (var i = 0; i < collectionsModel.count; i++) {
                        queueModel.append(collectionsModel.get(i));
                    }
            ...
            Item {
                ...
                Rectangle {
                    ...
                    Flickable {
                        ...
                        Item {
                            id: buttons
                            width: active_download == 0 ? downloadButton.width + externalButton.width + lyricsButton.width + artistButton.width + albumButton.width + favButton.width + collectionButton.width + pylButton.width + shareButton.width + buyButton.width + units.gu(36) + units.gu(6) : parent.width
                            height: parent.height
                            anchors.left: parent.left
                            anchors.leftMargin: units.gu(2)
                            Rectangle {
                            ...
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_fav == 1) {
                                            Scripts.remove_from_fav(id, 'collection');
                                        } else {
                                            Scripts.add_to_fav(id, 'collection');
                                        }
                                    }
                                }
                            }
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_lib == 1) {
                                            Scripts.remove_from_lib(id, albumId, artistId, uid, 'collection');
                                        } else {
                                            var song_data = [{
                                                'songID' : id,
                                                'songName' : title,
                                            ...
                                            Scripts.add_to_lib(song_data, 'collection');
                                        }
                                    }
                                }
                            }
        ...
        PullToRefresh {
            refreshing: collectionsModel.count == 0 && collections.finished == false
            onRefresh: get_lib()
        }
    }
    Scrollbar {
        flickableItem: collections
    }

    Item {
        id: indicat
        anchors.centerIn: parent
        opacity: collectionsModel.count == 0 && collections.finished == false ? 1 : 0
    ...
    Item {
        id: noitem
        visible: collectionsModel.count == 0 && collections.finished == true ? true : false
        ...
        Label {
            ...
            anchors.centerIn: parent
            fontSize: "large"
            text: i18n.tr("<b>You have not collected any songs.</b>")
        }
    }
}
