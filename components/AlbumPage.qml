...
Item {
    ...
    function album(albumId) {
        Scripts.album_songs(albumId);
        get_fav();
        get_lib();
    }

    function get_fav() {
        Scripts.get_fav_songs(uid, 'album');
    }
    function get_lib() {
        Scripts.get_lib_songs(uid, 'album');
    }
    ...
    ListView {
        ...
        delegate: ListItem.Empty {
            ...
            MouseArea {
                ...
                onClicked: {
                    queueModel.clear();
                    for (var i = 0; i < albumsongsModel.count; i++) {
                        queueModel.append(albumsongsModel.get(i));
                    }
            ...
            Item {
                ...
                Rectangle {
                    ...
                    Flickable {
                        ...
                        Item {
                            id: buttons
                            width: active_download == 0 ? downloadButton.width + externalButton.width + lyricsButton.width + artistButton.width + favButton.width + collectionButton.width + pylButton.width + shareButton.width + buyButton.width + units.gu(32) + units.gu(6) : parent.width
                            height: parent.height
                            anchors.left: parent.left
                            anchors.leftMargin: units.gu(2)
                            Rectangle {
                            ...
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_fav == 1) {
                                            Scripts.remove_from_fav(id, 'album');
                                        } else {
                                            Scripts.add_to_fav(id, 'album');
                                        }
                                    }
                                }
                            }
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        if (is_lib == 1) {
                                            Scripts.remove_from_lib(id, albumId, artistId, uid, 'album');
                                        } else {
                                            var song_data = [{
                                                'songID' : id,
                                                'songName' : title,
                                            ...
                                            Scripts.add_to_lib(song_data, 'album');
                                        }
                                    }
                                }
                            }
    ...
    Scrollbar {
        flickableItem: albumsongs
    }

    Item {
        id: indicat
        anchors.centerIn: parent
        opacity: albumsongsModel.count != 0 ? 0 : 1
